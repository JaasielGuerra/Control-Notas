...
CREATE TABLE IF NOT EXISTS `db_control_notas`.`alumno` (
  ...
  `encargado` INT NOT NULL COMMENT 'VALORES:\n\n1 = PADRE\n2 = MADRE',
...
CREATE TABLE IF NOT EXISTS `db_control_notas`.`libro` (
  ...
  `id_usuario` BIGINT NOT NULL,
  `condicion_libro` INT NOT NULL COMMENT '1 = buen estado\n2 = perdido\n3 = deteriorado',
...
CREATE TABLE IF NOT EXISTS `db_control_notas`.`detalle_lectura` (
  ...
  INDEX `fk_detalle_lectura_bimestre1_idx` (`id_bimestre` ASC),
...
CREATE TABLE IF NOT EXISTS `db_control_notas`.`detalle_listado` (
  `id_detalle_listado` BIGINT NOT NULL AUTO_INCREMENT,
  ...
  INDEX `fk_detalle_listado_bimestre1_idx` (`id_bimestre` ASC),
...
CREATE TABLE IF NOT EXISTS `db_control_notas`.`asignacion_materia` (
  ...
  INDEX `fk_asignacion_materia_seccion1_idx` (`id_seccion` ASC),
  ...
  CONSTRAINT `fk_asignacion_materia_materia1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_asignacion_materia_seccion1`
    FOREIGN KEY (`id_seccion`)
    REFERENCES `db_control_notas`.`seccion` (`id_seccion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- Funciones
-- -----------------------------------------------------
DROP FUNCTION IF EXISTS func_obtener_puntos_conducta_alumno;
...
DROP FUNCTION IF EXISTS func_obtener_porcentaje_asistencia_alumno;

DELIMITER $$
$$
CREATE FUNCTION func_obtener_porcentaje_asistencia_alumno(
...
DROP FUNCTION IF EXISTS func_alumno_tiene_libro_asignado;
DELIMITER $$
CREATE  FUNCTION `func_alumno_tiene_libro_asignado`( id_alumno BIGINT ) RETURNS int(11)
BEGIN RETURN(
  SELECT
...
DROP FUNCTION IF EXISTS func_consultar_libro_actual_alumno;
DELIMITER $$
CREATE FUNCTION `func_consultar_libro_actual_alumno`( id_alumno BIGINT ) RETURNS varchar(200) CHARSET utf8mb4
BEGIN RETURN (
    SELECT
...
DROP FUNCTION IF EXISTS db_control_notas.func_existe_plan_trabajo;
DELIMITER $$
...
DROP FUNCTION IF EXISTS func_obtener_id_ultimo_libro_alumno;
DELIMITER $$
CREATE FUNCTION `func_obtener_id_ultimo_libro_alumno`(
    id_alumno BIGINT
) RETURNS bigint(20)
BEGIN
    RETURN(
...
DROP PROCEDURE IF EXISTS proc_consulta_calificar_evaluaciones;
DELIMITER $$
CREATE  PROCEDURE `proc_consulta_calificar_evaluaciones`(
    id_bimestre LONG,
...
DROP PROCEDURE IF EXISTS proc_listar_alumnos_lectura;
DELIMITER $$
...
DROP TRIGGER IF EXISTS trig_cambiar_disponibilidad_libro;
DELIMITER $$
...
IF NEW.tipo_operacion = 1 THEN 

	UPDATE
		libro l
	SET l.disponibilidad = 2 
	WHERE
		l.id_libro = NEW.id_libro;

END IF;

IF NEW.tipo_operacion = 3 THEN 

	UPDATE
		libro l
	SET l.disponibilidad = 1 
	WHERE
		l.id_libro = NEW.id_libro;
